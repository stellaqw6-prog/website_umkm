...
export function ProductReviews({ productId, productSlug }: { productId: number; productSlug: string }) {
  ...
  return (
    <div className="mt-16 border-t border-gray-100 pt-10 dark:border-stone-800">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-xl font-bold text-gray-900 dark:text-stone-100">Ulasan Pembeli ({total})</h2>
        {user && !showForm && (
      ...
      {!user && (
        <p className="text-sm text-gray-500 mb-6 dark:text-stone-400">
      ...
      {showForm && (
        <motion.form
          ...
          className="bg-gray-50 rounded-2xl p-5 mb-8 space-y-4 dark:bg-stone-900"
        >
          <div>
            <label className="text-sm font-medium text-gray-700 mb-2 block dark:text-stone-300">Rating Kamu</label>
          ...
          <div>
            <label className="text-sm font-medium text-gray-700 mb-2 block dark:text-stone-300">Ulasan Kamu</label>
      ...
      {loading ? (
        <div className="flex justify-center py-10 text-gray-400 dark:text-stone-500"><Loader2 className="animate-spin" size={24} /></div>
      ) : total === 0 ? (
        <p className="text-gray-400 text-sm py-6 dark:text-stone-500">Belum ada ulasan untuk produk ini. Jadilah yang pertama!</p>
      ) : (
        <>
          {/* Distribution bar */}
          <div className="space-y-1.5 mb-8 max-w-sm">
            {distribution.map((d) => (
              <div key={d.star} className="flex items-center gap-2 text-xs">
                <span className="w-8 text-gray-500 dark:text-stone-400">{d.star} ★</span>
                <div className="flex-1 h-2 bg-gray-100 rounded-full overflow-hidden dark:bg-stone-800">
                  <div className="h-full bg-yellow-400 rounded-full" style={{ width: `${(d.count / maxCount) * 100}%` }} />
                </div>
                <span className="w-6 text-gray-400 text-right dark:text-stone-500">{d.count}</span>
              </div>
            ))}
          </div>
            ...
            {reviews.map((r) => (
              <div key={r.id} className="border-b border-gray-50 pb-5">
                <div className="flex items-center gap-3 mb-2">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-gray-900 dark:text-stone-100">{r.userName ?? "Pembeli"}</p>
                    <div className="flex items-center gap-0.5">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <Star key={i} size={12} className={i < r.rating ? "fill-yellow-400 text-yellow-400" : "text-gray-200"} />
                      ))}
                      <span className="text-xs text-gray-400 ml-1.5 dark:text-stone-500">{new Date(r.createdAt).toLocaleDateString("id-ID")}</span>
                    </div>
                  </div>
                </div>
                <p className="text-sm text-gray-600 leading-relaxed dark:text-stone-400">{r.content}</p>
              </div>
            ))}
          </div>
        </>
