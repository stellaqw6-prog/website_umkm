...
export function CheckoutSuccessPage() {
  ...
  return (
    <section className="py-12 bg-gray-50 min-h-screen dark:bg-gray-900">
      <div className="container mx-auto px-4 max-w-lg">
        ...
        {!loading && (
          <>
            <div className="bg-white rounded-2xl border border-gray-100 p-5 mb-4 text-center dark:bg-gray-900 dark:border-gray-800">
              <p className="text-xs text-gray-500 dark:text-gray-400">Total Keseluruhan</p>
              <p className="text-2xl font-extrabold text-blue-600">{formatCurrency(total)}</p>
              <p className="text-xs text-gray-400 mt-1 dark:text-gray-500">Cukup transfer/bayar 1 kali untuk total ini</p>
            </div>

            <div className="space-y-3 mb-6">
              {orders.map((order, i) => (
                <motion.div key={order.orderNumber} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}>
                  <Link href={`/pesanan/${order.orderNumber}`} className="flex items-center gap-3 bg-white rounded-xl border border-gray-100 p-4 hover:shadow-md transition-all dark:bg-gray-900 dark:border-gray-800">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-400 dark:text-gray-500">{formatCurrency(Number(order.grandTotal))}</p>
                    </div>
