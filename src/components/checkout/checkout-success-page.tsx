        ...
        {!loading && (
          <>
            <div className="bg-white rounded-2xl border border-gray-100 p-5 mb-4 text-center">
              ...
              <p className="text-xs text-gray-400 mt-1">Cukup transfer/bayar 1 kali untuk total ini</p>
            </div>

            <div className="space-y-3 mb-6">
              {orders.map((order, i) => (
                <motion.div key={order.orderNumber} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}>
                  <Link href={`/pesanan/${order.orderNumber}`} className="flex items-center gap-3 bg-white rounded-xl border border-gray-100 p-4 hover:shadow-md transition-all">
                    <div className="w-10 h-10 rounded-xl bg-blue-50 flex items-center justify-center flex-shrink-0">
                      <Store size={18} className="text-blue-600" />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className="font-mono font-semibold text-gray-900 text-sm">{order.orderNumber}</p>
                      <p className="text-xs text-gray-400">{formatCurrency(Number(order.grandTotal))}</p>
                    </div>
