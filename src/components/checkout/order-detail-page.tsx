  ...
  if (notFound || !order) {
    return (
      <div className="min-h-[60vh] flex items-center justify-center text-center px-4">
        <div>
          <h2 className="text-xl font-bold text-gray-900 mb-2 dark:text-stone-100">Pesanan tidak ditemukan</h2>
  ...
  return (
    <section className="py-10 bg-gray-50 min-h-screen dark:bg-stone-900">
      <div className="container mx-auto px-4 max-w-2xl">
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-6 space-y-6 dark:bg-stone-900 dark:border-stone-800">
          <div className="flex items-center justify-between">
            <span className="text-sm text-gray-500 dark:text-stone-400">Status Pesanan</span>
            <Badge variant={statusVariant[order.status] ?? "secondary"}>{statusLabel[order.status] ?? order.status}</Badge>
          </div>

          <div>
            <h3 className="text-sm font-semibold text-gray-700 mb-3 flex items-center gap-1.5 dark:text-stone-300"><Package size={15} /> Produk</h3>
            <div className="space-y-3">
              {items.map((item) => (
                <div key={item.id} className="flex gap-3 text-sm">
                  <img src={item.productImage ?? ""} alt={item.productName} className="w-12 h-14 object-cover rounded-lg bg-gray-100 flex-shrink-0 dark:bg-stone-800" />
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-gray-900 line-clamp-1 dark:text-stone-100">{item.productName}</p>
                    {item.variantName && <p className="text-gray-400 text-xs dark:text-stone-500">Varian: {item.variantName}</p>}
                    <p className="text-gray-500 text-xs dark:text-stone-400">{item.quantity} x {formatCurrency(Number(item.price))}</p>
                  </div>
                  <span className="font-semibold text-gray-900 whitespace-nowrap dark:text-stone-100">{formatCurrency(Number(item.subtotal))}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="space-y-2 text-sm">
            <div className="flex justify-between"><span className="text-gray-500 dark:text-stone-400">Subtotal</span><span>{formatCurrency(Number(order.totalAmount))}</span></div>
          ...
          <div className="space-y-3 text-sm">
            <div className="flex items-start gap-2"><MapPin size={15} className="text-gray-400 mt-0.5 flex-shrink-0 dark:text-stone-500" /><span className="text-gray-600 dark:text-stone-400">{order.shippingAddress}</span></div>
            <div className="flex items-center gap-2"><CreditCard size={15} className="text-gray-400 flex-shrink-0 dark:text-stone-500" /><span className="text-gray-600 dark:text-stone-400">{order.paymentMethod}</span></div>
          </div>
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white rounded-2xl border border-blue-100 p-6 mt-4 dark:bg-stone-900 dark:border-blue-900/40">
            ...
            <div className="rounded-xl border border-blue-100 bg-blue-50/40 p-4 dark:border-blue-900/40 dark:bg-blue-950/20">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="font-mono font-bold text-gray-900 text-lg dark:text-stone-100">{paymentMethod.accountNumber}</p>
                  <p className="text-xs text-gray-500 mt-0.5 dark:text-stone-400">a.n. {paymentMethod.accountName}</p>
                </div>
                <button
              ...
              {paymentMethod.qrImage && (
                <div className="mt-3 flex items-center gap-3">
                  <img src={paymentMethod.qrImage} alt={`QR ${paymentMethod.name}`} className="w-24 h-24 rounded-lg border border-gray-200 object-contain bg-white dark:bg-stone-900 dark:border-stone-700" />
              ...
              {paymentMethod.instructions && (
                <p className="text-xs text-gray-500 mt-3 leading-relaxed dark:text-stone-400">{paymentMethod.instructions}</p>
              )}
            </div>
            ...
            <div className="mt-4 pt-4 border-t border-gray-100 dark:border-stone-800">
              {order.paymentProofUrl ? (
                ...
                <div>
                  <h4 className="text-sm font-semibold text-gray-900 mb-2 flex items-center gap-1.5 dark:text-stone-100"><Upload size={14} /> Upload Bukti Transfer</h4>
                  <p className="text-xs text-gray-500 mb-3 dark:text-stone-400">Setelah transfer, upload screenshot/foto bukti pembayaran di sini supaya pesananmu segera diproses.</p>
                  <label
                    className={`flex flex-col items-center justify-center gap-2 border-2 border-dashed rounded-xl py-8 cursor-pointer transition-colors ${
                      uploading ? "border-gray-200 bg-gray-50 dark:border-stone-700 dark:bg-stone-900" : "border-blue-200 hover:border-blue-400 hover:bg-blue-50/50 dark:border-blue-900 dark:hover:bg-blue-950/20"
                    }`}
                  >
                    {uploading ? (
                      <>
