        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {testimonials.map((testimonial, i) => (
            <motion.div
              ...
              whileHover={{ y: -4 }}
              className="bg-gray-50 rounded-2xl p-6 relative group hover:shadow-lg transition-all dark:bg-gray-900 dark:hover:shadow-black/40"
            >
              <Quote className="absolute top-4 right-4 text-gray-200 w-10 h-10 group-hover:text-blue-100 transition-colors dark:text-gray-800 dark:group-hover:text-blue-950" />
              <div className="flex items-center gap-1 mb-3">
                {Array.from({ length: 5 }).map((_, j) => (
                  <Star
                    key={j}
                    size={14}
                    className={j < testimonial.rating ? "text-yellow-500 fill-yellow-500" : "text-gray-300 dark:text-gray-700"}
                  />
                ))}
              </div>
              ...
              <div className="flex items-center gap-3 border-t border-gray-200 pt-4 dark:border-gray-800">
                ...
                <div>
                  <p className="font-semibold text-gray-900 text-sm dark:text-gray-100">{testimonial.name}</p>
