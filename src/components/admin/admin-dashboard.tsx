...
export function AdminDashboard() {
  ...
  if (!stats) {
    return <p className="text-center py-24 text-gray-400 text-sm">Gagal memuat statistik dashboard.</p>;
  }
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-gray-500 text-sm mt-1">Ringkasan bisnis Anda berdasarkan data pesanan asli</p>
      </motion.div>
                  ...
                  {stats.monthlyRevenue.map((m) => (
                    <div key={m.month} className="flex-1 flex flex-col items-center gap-2">
                      <div className="w-full flex items-end justify-center" style={{ height: "220px" }}>
                        ...
                        >
                          <span className="absolute -top-6 left-1/2 -translate-x-1/2 text-[10px] font-medium text-gray-500 opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
            ...
            <CardContent className="space-y-3">
              {stats.recentOrders.length === 0 ? (
                ...
                stats.recentOrders.map((order) => (
                  <div key={order.orderNumber} className="flex items-center justify-between p-3 rounded-xl hover:bg-gray-50 transition-colors">
                    ...
                    <span className="text-sm font-semibold text-gray-900 whitespace-nowrap ml-2">{formatCurrency(order.amount)}</span>
                  </div>
                ))
              )}
            </CardContent>
