  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-stone-100">FAQ</h1>
          <p className="text-gray-500 text-sm mt-1 dark:text-stone-400">Kelola pertanyaan yang sering diajukan</p>
        </div>
            ...
            <motion.div key={faq.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }} className="border border-gray-100 rounded-xl overflow-hidden dark:border-stone-800">
              ...
              {openId === faq.id && (
                <div className="px-4 pb-4 pl-4">
                  <p className="text-sm text-gray-500 leading-relaxed mb-3 dark:text-stone-400">{faq.answer}</p>
                  <div className="flex items-center gap-1">
                    <button onClick={() => openEdit(faq)} className="flex items-center gap-1 text-xs font-medium text-gray-600 hover:text-blue-600 py-1.5 px-3 rounded-lg hover:bg-gray-50 transition-all dark:hover:bg-stone-800 dark:text-stone-400"><Edit size={13} /> Edit</button>
                    <button onClick={() => handleDelete(faq)} className="flex items-center gap-1 text-xs font-medium text-gray-600 hover:text-red-600 py-1.5 px-3 rounded-lg hover:bg-gray-50 transition-all dark:hover:bg-stone-800 dark:text-stone-400"><Trash2 size={13} /> Hapus</button>
                  </div>
                </div>
              )}
            </motion.div>
      ...
      <AdminModal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? "Edit FAQ" : "Tambah FAQ"}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div><label className="text-sm font-medium text-gray-700 mb-1.5 block dark:text-stone-300">Pertanyaan</label><Input required value={form.question} onChange={(e) => setForm({ ...form, question: e.target.value })} /></div>
          <div><label className="text-sm font-medium text-gray-700 mb-1.5 block dark:text-stone-300">Jawaban</label><Textarea required rows={4} value={form.answer} onChange={(e) => setForm({ ...form, answer: e.target.value })} /></div>
          <div><label className="text-sm font-medium text-gray-700 mb-1.5 block dark:text-stone-300">Kategori</label><Input value={form.category} onChange={(e) => setForm({ ...form, category: e.target.value })} placeholder="Pemesanan, Pengiriman, dll" /></div>
