  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-stone-100">Testimoni</h1>
          <p className="text-gray-500 text-sm mt-1 dark:text-stone-400">Kelola ulasan dan testimoni pelanggan</p>
        </div>
      ...
      <Card>
        ...
        <CardContent className="pt-4">
          ...
          : filtered.length === 0 ? <p className="text-center py-16 text-gray-400 text-sm dark:text-stone-500">Belum ada testimoni.</p>
          : (
            <div className="grid md:grid-cols-2 gap-4">
              {filtered.map((t, i) => (
                <motion.div key={t.id} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="border border-gray-100 rounded-xl p-4 hover:shadow-md transition-all dark:border-stone-800">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-semibold text-gray-900 dark:text-stone-100">{t.name}</p>
                        <p className="text-xs text-gray-500 dark:text-stone-400">{t.role}</p>
                      </div>
                    </div>
                  ...
                  <p className="text-sm text-gray-600 mb-3 leading-relaxed line-clamp-3 dark:text-stone-400">&ldquo;{t.content}&rdquo;</p>
                  <div className="flex items-center gap-1 pt-2 border-t border-gray-50">
                    <button onClick={() => openEdit(t)} className="flex-1 flex items-center justify-center gap-1 text-xs font-medium text-gray-600 hover:text-blue-600 py-1.5 rounded-lg hover:bg-gray-50 transition-all dark:hover:bg-stone-800 dark:text-stone-400"><Edit size={13} /> Edit</button>
                    <button onClick={() => handleDelete(t)} className="flex-1 flex items-center justify-center gap-1 text-xs font-medium text-gray-600 hover:text-red-600 py-1.5 rounded-lg hover:bg-gray-50 transition-all dark:hover:bg-stone-800 dark:text-stone-400"><Trash2 size={13} /> Hapus</button>
                  </div>
                </motion.div>
              ))}
            </div>
      ...
      <AdminModal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? "Edit Testimoni" : "Tambah Testimoni"}>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div><label className="text-sm font-medium text-gray-700 mb-1.5 block dark:text-stone-300">Nama</label><Input required value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /></div>
          <div><label className="text-sm font-medium text-gray-700 mb-1.5 block dark:text-stone-300">Peran (mis. Pelanggan Setia)</label><Input value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })} /></div>
          <div><label className="text-sm font-medium text-gray-700 mb-1.5 block dark:text-stone-300">Isi Testimoni</label><Textarea required rows={3} value={form.content} onChange={(e) => setForm({ ...form, content: e.target.value })} /></div>
          <div>
            <label className="text-sm font-medium text-gray-700 mb-1.5 block dark:text-stone-300">Rating</label>
