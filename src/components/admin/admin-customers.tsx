  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        <h1 className="text-2xl font-bold text-gray-900 dark:text-stone-100">Pelanggan</h1>
        <p className="text-gray-500 text-sm mt-1 dark:text-stone-400">Data pelanggan yang sudah mendaftar dan riwayat belanjanya</p>
      </motion.div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b border-gray-100 dark:border-stone-800">
                    ...
                    <th className="text-left py-3 px-4 text-xs font-semibold text-gray-500 uppercase dark:text-stone-400">Kontak</th>
                    <th className="text-center py-3 px-4 text-xs font-semibold text-gray-500 uppercase dark:text-stone-400">Membership</th>
                    <th className="text-right py-3 px-4 text-xs font-semibold text-gray-500 uppercase dark:text-stone-400">Pesanan</th>
                    <th className="text-right py-3 px-4 text-xs font-semibold text-gray-500 uppercase dark:text-stone-400">Total Belanja</th>
                    ...
                    <tr key={customer.id} className="border-b border-gray-50 hover:bg-gray-50/50 dark:border-stone-800/60 dark:hover:bg-stone-800/40 transition-colors">
                      <td className="py-3 px-4">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-medium text-gray-900 text-sm dark:text-stone-100">{customer.name}</span>
                        </div>
                      </td>
                      <td className="py-3 px-4">
                        <div className="text-xs text-gray-500 dark:text-stone-400">{customer.email}</div>
                        {customer.phone && <div className="text-xs text-gray-500 dark:text-stone-400">{customer.phone}</div>}
                      </td>
                      <td className="py-3 px-4 text-center">
                        <Badge variant={membershipVariant[customer.membership]} className="text-[10px] capitalize">{customer.membership}</Badge>
                      </td>
                      <td className="py-3 px-4 text-sm text-right font-medium text-gray-900 dark:text-stone-100">{customer.orders}</td>
                      <td className="py-3 px-4 text-sm text-right font-semibold text-gray-900 dark:text-stone-100">{formatCurrency(customer.totalSpent)}</td>
                      <td className="py-3 px-4 text-sm text-gray-500 dark:text-stone-400">{new Date(customer.joined).toLocaleDateString("id-ID")}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
