  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-gray-500 text-sm mt-1">Data pelanggan yang sudah mendaftar dan riwayat belanjanya</p>
      </motion.div>
                ...
                <tbody>
                  {filtered.map((customer) => (
                    <tr key={customer.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="py-3 px-4 text-sm text-right font-semibold text-gray-900">{formatCurrency(customer.totalSpent)}</td>
                      <td className="py-3 px-4 text-sm text-gray-500">{new Date(customer.joined).toLocaleDateString("id-ID")}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
