      ...
      {loading ? (
        ...
        <>
          ...
          <div>
            ...
            {ewallets.length === 0 ? (
              <Card><CardContent className="py-10 text-center text-sm text-gray-400">Belum ada metode e-wallet.</CardContent></Card>
            ) : (
          ...
          <div>
            ...
            {banks.length === 0 ? (
              <Card><CardContent className="py-10 text-center text-sm text-gray-400">Belum ada rekening bank.</CardContent></Card>
            ) : (
              <div className="grid md:grid-cols-2 gap-4">
                {banks.map((m, i) => (
                  <PaymentMethodCard key={m.id} m={m} i={i} onEdit={openEdit} onDelete={handleDelete} onToggle={handleToggleActive} />
          ...
          <div>
            ...
            {cods.length === 0 ? (
              <Card><CardContent className="py-10 text-center text-sm text-gray-400">Belum ada metode COD.</CardContent></Card>
            ) : (
              <div className="grid md:grid-cols-2 gap-4">
                {cods.map((m, i) => (
                  <PaymentMethodCard key={m.id} m={m} i={i} onEdit={openEdit} onDelete={handleDelete} onToggle={handleToggleActive} />
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                className="w-full h-10 rounded-lg border border-gray-200 px-3 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                value={form.type}
          ...
          <div>
            <label className="text-sm font-medium text-gray-700 mb-1.5 block">Provider (kode)</label>
            ...
            <p className="text-xs text-gray-400 mt-1">Kode singkat, huruf kecil, tanpa spasi.</p>
          </div>
          ...
          {form.type === "cod" ? (
            ...
            <>
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="text-sm font-medium text-gray-700 mb-1.5 block">Atas Nama</label>
                  <Input required value={form.accountName} onChange={(e) => setForm({ ...form, accountName: e.target.value })} placeholder="Nama pemilik akun" />
                </div>
              </div>

              <div>
                <label className="text-sm font-medium text-gray-700 mb-1.5 block flex items-center gap-1"><QrCode size={14} /> URL Gambar QR (opsional)</label>
          ...
          <div>
            <label className="text-sm font-medium text-gray-700 mb-1.5 block">Instruksi Tambahan (opsional)</label>
        ...
        <CardContent className="p-5">
          <div className="flex items-start justify-between gap-3 mb-3">
            <div className="flex items-center gap-3 min-w-0">
              <div className="w-10 h-10 rounded-xl bg-gray-50 flex items-center justify-center text-lg flex-shrink-0">
                {providerIcons[m.provider] ?? <GripVertical size={18} className="text-gray-400" />}
              </div>
              <div className="min-w-0">
                <p className="font-semibold text-gray-900 truncate">{m.name}</p>
                <p className="text-xs text-gray-500">{m.accountName}</p>
              </div>
            </div>
          ...
          {m.type !== "cod" && (
            <div className="bg-gray-50 rounded-xl px-3 py-2 mb-3">
              <p className="text-xs text-gray-500">{m.type === "bank" ? "No. Rekening" : "No. HP"}</p>
              <p className="font-mono font-semibold text-gray-900">{m.accountNumber}</p>
            </div>
          )}
          {m.type === "cod" && m.instructions && (
            <div className="bg-gray-50 rounded-xl px-3 py-2 mb-3">
              <p className="text-xs text-gray-500 leading-relaxed">{m.instructions}</p>
            </div>
          )}
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => onEdit(m)} className="flex items-center gap-1 text-xs font-medium text-gray-600 hover:text-blue-600 py-1.5 px-3 rounded-lg hover:bg-gray-50 transition-all">
              <Edit size={13} /> Edit
            </button>
            <button onClick={() => onToggle(m)} className="text-xs font-medium text-gray-600 hover:text-blue-600 py-1.5 px-3 rounded-lg hover:bg-gray-50 transition-all">
              {m.isActive ? "Nonaktifkan" : "Aktifkan"}
            </button>
            <button onClick={() => onDelete(m)} className="flex items-center gap-1 text-xs font-medium text-gray-600 hover:text-red-600 py-1.5 px-3 rounded-lg hover:bg-gray-50 transition-all">
