...
export function SellerDashboard() {
  ...
  if (!stats) return <p className="text-center py-24 text-gray-400 text-sm">Gagal memuat data.</p>;
  ...
  return (
    <div className="space-y-6">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-gray-500 text-sm mt-1">Ringkasan performa toko kamu</p>
      </motion.div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {stats.recentOrders.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-10 text-gray-400"><Inbox size={28} className="mb-2 opacity-40" /><p className="text-sm">Belum ada pesanan masuk.</p></div>
          ) : (
            stats.recentOrders.map((order) => (
              <div key={order.orderNumber} className="flex items-center justify-between p-3 rounded-xl hover:bg-gray-50 transition-colors">
                ...
                <span className="text-sm font-semibold text-gray-900">{formatCurrency(Number(order.grandTotal))}</span>
              </div>
            ))
          )}
        </CardContent>
